...
from .models import Contact, Factory, Network, Product, Seller
...
class CityFilter(SimpleListFilter):
    ...
    def lookups(self, request: Any, model_admin: Any) -> list[tuple[Any, str]]:
        contacts_ids = model_admin.get_queryset(request).all().values('contact')
        queryset = Contact.objects.filter(id__in=contacts_ids).order_by('city').distinct('city')
