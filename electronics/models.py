...
class Contact(models.Model):
    email = models.EmailField(max_length=1000, unique=True, verbose_name='Почта')
...
class Seller(models.Model):
    ...
    name = models.CharField(max_length=50, verbose_name='Название', unique=True)
    contact = models.ForeignKey(Contact, on_delete=models.CASCADE, verbose_name='Контакты')
    supplier_choice = models.CharField(choices=SupplierChoice.choices, default=SupplierChoice.NETWORK,
                                       verbose_name='Поставщик')
    supplier_factory = models.ForeignKey(Factory, on_delete=models.CASCADE, verbose_name='Поставщик (фабрика)',
                                         blank=True, null=True)
    supplier_network = models.ForeignKey(Network, on_delete=models.CASCADE, verbose_name='Поставщик (розничная сеть)',
                                         blank=True, null=True)
    debt = models.PositiveIntegerField(verbose_name='Задолженность', default=0)
    created_at = models.DateTimeField(blank=True, auto_now_add=True, verbose_name='Дата создания')
...
class Prod_sell(models.Model):
    product = models.ForeignKey(Product, on_delete=models.CASCADE, verbose_name='Продукт')
    seller = models.ForeignKey(Seller, on_delete=models.CASCADE, verbose_name='Предприниматель')
